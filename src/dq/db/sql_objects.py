"""Replaceable database objects — functions whose definition lives in one place.

A migration that inlines a 300-line PL/pgSQL body freezes it at the revision that created it.
Changing it then means either editing an applied migration (which never reaches a database that
already ran it) or pasting a second copy into a new one (which drifts from the first). Both are
worse than keeping the definition here and having every migration that needs it re-apply it.

``CREATE OR REPLACE`` makes this safe: a fresh database gets the current text from migration 0004,
an existing one gets it again from 0006, and both converge on whatever this module says today.
"""
...
from dq.db.migration_support import execute_raw
...
DECLARE
    ...
    -- Without them a feed rule cannot know which source and period it is being asked about, so it
    -- would have to evaluate every declared expectation on every run — and the same missing period
    -- would then re-fire under a fresh scope_key each time, which is exactly the duplication the
    -- uniqueness key was rekeyed to prevent (research.md D4).
    engine_params   text[] := ARRAY[
        'batch_id','as_of_date','reference_watermark',
        'scope_source_system_id','scope_period_start','scope_period_end'
    ];
...
BEGIN
    ...
    ----------------------------------------------------------------------
    -- Rule 8b: no postfix cast directly on a bound parameter.
    --
    -- `:name::type` passes every other check here and then CANNOT BE
    -- EXECUTED. The driver declines to bind `:name` when a colon follows it
    -- -- that is how it tells a cast from a marker -- so the marker survives
    -- into the statement and PostgreSQL rejects it at run time. The engine
    -- records ERRORED and carries on, which is safe but silent: the rule
    -- contributes nothing for the whole scope and a steward sees a quiet gap.
    --
    -- Checked against the raw predicate, since `stripped` has had casts
    -- removed already.
    ----------------------------------------------------------------------
    IF predicate_sql ~ ':[a-zA-Z_][a-zA-Z0-9_]*\\s*::' THEN
        RAISE EXCEPTION
            'Rule 8b (no postfix cast on a bound parameter): a parameter is followed by ::, which '
            'leaves it unbound at execution time. Write CAST(:name AS <type>) instead.'
            USING ERRCODE = 'check_violation';
    END IF;
...
def define(conn: Connection, sql: str, *, commercial: str, dq: str) -> None:
    """Create or replace one function, with its ``%`` placeholders intact.

    ``execute_raw`` goes straight to the driver cursor with no parameters, because both SQLAlchemy's
    ``text()`` and psycopg's placeholder parser treat ``%`` as a bind marker — and ``RAISE`` format
    specifiers are full of them.
    """
    execute_raw(conn, sql.replace("@DQ@", dq).replace("@COMMERCIAL@", commercial))


def define_all(conn: Connection, *, commercial: str, dq: str) -> None:
    """Apply every replaceable function in dependency order."""
    for sql in (REJECT_MUTATION_FN, VALIDATE_PREDICATE_FN, VALIDATE_TRIGGER_FN):
        define(conn, sql, commercial=commercial, dq=dq)
